 ...
 *
 * The two grounds are not a style choice. The Yadlo wordmark is white and needs a dark ground; the
 * Morges and Préverenges logos are dark artwork on transparency and need a light one. Tinting either
 * to resolve that is not available — a logo is never recoloured — so the backers sit on a white
 * plate and keep their own colours.
 */
@Composable
fun SplashScreen(
    onFinish: () -> Unit,
    ...
    Box(
        ...
        modifier =
            modifier
                .fillMaxSize()
                .background(MaterialTheme.appColors.brandGround),
    ) {
        // The beach photograph belongs here, behind the wordmark and under a scrim. Until it exists
        // the brand ground is the whole background, which is why that colour is a real token rather
        // than a placeholder to swap out.
        ...
        ComposeImage(
            ...
            modifier = Modifier.padding(horizontal = MaterialTheme.spacing.xxl).width(WORDMARK_WIDTH),
        )
 ...
 */
private val BACKER_LOGO_HEIGHT = 44.dp
