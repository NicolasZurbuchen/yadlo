 ...
 */
private class StandsScreenStateProvider : PreviewParameterProvider<StandsUiModel> {
    override val values =
        sequenceOf(
            StandsUiModel(
                isLoading = true,
...
/**
 * The dark rendering is where the chips need checking: they sit on the bar's own surface rather
 * than on the page, so the selected chip's ink has to clear a ground it did not before.
 */
@PreviewThemes
@Composable
private fun StandsScreenPreview(
    @PreviewParameter(StandsScreenStateProvider::class) state: StandsUiModel,
) {
    YadloPreview {
